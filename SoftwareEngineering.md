...
* 面: 基于公司和部门的战略规划，将大量的需求线进行归类和合并，相互交织成为整个需求的面。

软件设计和开发的质量非常难于被精确的衡量。

针对复杂工程问题的通用思路是通过模块化来简化问题
* 逐步分解为细粒度的、相对独立的模块
* 通过简单的接口集成模块所提供的功能
...
* 功能
* 数据
* 消耗：需要消耗大量计算、存储和网络资源的情况，需要划分为不同的系统

软件的边际成本为0，即能够以零成本进行复制拷贝。

如果用户相同，但是功能和数据完全不同，可以将这些功能划分为不同的系统，并通过单点登录使得用户可以在多个系统之前切换

如果部分功能完全相同或者大部分功能相同，可以采用将这部分功能构建为
* 独立的服务
* 公共的库
* 通用的模块
* 依赖的功能（类）

如果数据相同，而功能和/或用户不同，可以采用数据库的主从方式。
...
* Don’t Repeat Yourself (DRY)。
  ...
  [DRY](http://en.wikipedia.org/wiki/KISS_principle)
...
* Program to an interface, not an implementation。
  面向接口而不是实现编程。这是设计模式中最根本的哲学，与 Composition over inheritance（组合优于继承）原则一起被称为23个经典设计模式中的设计原则。模块之间依赖于接口，而不是实现。接口是抽象的和稳定的，而实现则是多种多样的。该原则可以认为是依赖倒置原则的另一种描述形式。，就是这个原则的的另一种样子。
...
* Command-Query Separation (CQS) – 命令-查询分离原则。
 ...
 [COS](http://en.wikipedia.org/wiki/Command-query_separation)

 CQRS stands for Command Query Responsibility Segregation

* 接口隔离原则（Interface Segregation Principle:ISP）	1.使用多个专门的接口比使用单一的总接口总要好。换而言之，从一个客户类的角度来讲：一个类对另外一个类的依赖性应当是建立在最小接口上的。2．过于臃肿的接口是对接口的污染。不应该强迫客户依赖于它们不用的方法。
...
* High Cohesion and Low Loose coupling  – 高内聚， 低耦合
...
合成/聚合复用原则（Composite/Aggregate Reuse Principle,CARP）	尽量使用合成/聚合，尽量不要使用类继承。
* 聚合：表示一种弱的拥有关系，体现的是A 对象可以包含B对象，但B 对象不是A对象的一部分。contanis-a
* 合成：一种强的拥有关系，提现了严格的部分和整体的关系，部分和整体的生存周期一致。has-a)

迪米特法则（Law of Demeter）又叫作最少知识原则（The Least Knowledge Principle），一个类对于其他类知道的越少越好，就是说一个对象应当对其他对象有尽可能少的了解,只和朋友通信，不和陌生人说话强调类之间的松耦合。即：如果两个类不必彼此直接通信，那么着两个类就不应当发送直接的相互作用。如果其中一个类需要调用另一个类的某一个方法的话，可以通过第三者转发这个调用。
...
极限编程（XP）。这些方法学是不同于瀑布方法的一堆动作，产生背景是预先很难确定所有的需求，特别是需求经常变化的情况。要想预先创建一个覆盖领域所有方面的完整模型确实很困难。需要做出很多的思考，而且开始时常不能看到涉及到的所有的问题，也不能预见设计中某些带有负面影响或错误的部分。敏捷方法试图解决的另一个问题被称为“分析瘫痪”，团队成员会因为害怕做出任何设计决定而无所事事。。尽管敏捷方法的倡导者承认设计决定的重要性，但他们反对预先设计。相反，他们使用大量灵活的实现，通过由业务涉众持续参与的迭代开发和许多重构，开发团队更多地学习到了客户的领域知识，从而能够产出满足客户需要的软件。
...
一般采用"增量开发"（incremental development）划分迭代。
...
[https://zhuanlan.zhihu.com/p/400402095](分不清Kanban和看板的人只剩你了……)
Kanban VS 看板
