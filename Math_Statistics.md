...
随机事件 <--> 随机变量，随机事件是随机变量的特殊情况，随机事件是从镜头的观点来分析研究现象，而随机变量则是一种动态的观点

具体的实验 <--> 抽象的数，1）概括性，涵盖更广的场景；2）便于引入复杂的数学计算

随机变量 大写英文， 随机变量的取值或者具体的实验结果 小写英文，其没有随机性

随机变量的分布，distribution， distribution law

1、离散均匀分布，古典概型

2、二项分布 X~Ｂ(n,p),两个主要前提：1）各次实验的条件是稳定的，以确保p不变；2）各次实验的独立性

3、Poisson分布 ：描述稀有事件发生的概率

分布是用来计算随机事件的概率

Poisson分布 Poisson(lamda):用于表示在一定的时间或空间内出现的事件个数这种场合

Poisson逼近定理，用来近似二项分布 B(n,p) n很大，np较小

负二项分布：负指数二项展开式

超几何分布对应不放回；二项式分布对应有放回

生成函数，矩母函数Ｍ(t), 随机变量X的特征函数

随机变量X的分布函数F(x),f(x)概率密度函数

F(x)连续不一定是连续型随机变量，只有f(x)连续才行

均匀分布U(a,b) Uniform，二维的R(a,b) ,rectangle

指数分布，
* 无后效性 P(X>x+t|X>t)=P(X>s，用于描述元件的寿命
* P(x<X<=x+h)/h-> lemada
...
经常使用的有三种:皮尔森（pearson）相关系数和斯皮尔曼（spearman）相关系数和肯德尔（kendall）相关系数.皮尔森相关系数是衡量线性关联性的程度，p的一个几何解释是其代表两个变量的取值根据均值集中后构成的向量之间夹角的余弦.