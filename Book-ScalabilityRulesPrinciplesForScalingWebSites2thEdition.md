...
Charles Perrow的正态事故理论假设现代耦合系统所固有的复杂性使得故障不可避免。这些系统所固有的相互耦合会使得之间的交互迅速恶化，而没有给人类或者控制系统留下成功交互的机会。
...
For any major issue that we experience, we believe an organization should attack that issue with a postmortem process that addresses the problem in three distinct but easily described phases 我们相信一个组织应该通过事后处理流程来主攻我们所经历的任何主要问题，解决这问题的流程可用划分为三个迥异但易于描述的阶段。
