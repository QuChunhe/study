...
| :------------ | :------------ |  
|Pod |Pods are self-contained sets of functionality containing app servers, persistent　storage (such as a database or other persistent and shared file system), or　both. Pods are most often splits along the Z axis, as in a split of customers　into separate pods. Pod is sometimes used interchangeably with the term swim　lane. It has also been used interchangeably with the term pool when referring　to Web or application services.Pods是自包含的功能集合，包括应用服务器或者持久存储（例如数据库或者其他持久的和共享的文件系统），或者同时包含两者。Pods通常是沿着Z轴拆分，就像将客户拆分到相互分离的pods中。Pody有时也与术语泳道相互交换使用。当指的是Web或者应用时，也可以与术语池相互交换使用。|
|Cluster | Clusters are sometimes used for Web and application servers in the same fashion as a “pool,” identified next. In these cases a cluster refers to an X axis scale of similar functionality or purpose configured such that all nodes or participants are “active.” Often a cluster will share some sort of distributed state above and beyond that of a pool, but this state can cause scalability bottlenecks under high transaction volumes. Clusters might also refer to active/passive configuration where one or more devices sit “passive” and become “active” on the failure of a peer device.针对于Web或者应用服务器，有时会以像池一样的方式使用集群。 在这些例子中，一个集群指的是通过配置所有的节点或者参与者处于活跃状态来实现针对相似功能或者目的X轴扩展。通常一个集群会共享一些类型的状态，这些状态在池之上或者超过池，并且这些状态在大量事务时能够引起可扩展性瓶颈。集群可能也指的是活跃/消极配置，其一个或者多个设备节点处于消极状态，并且当对等设备故障时变为活跃状态。|  
|Pool | Pools are servers that group similar functionality or potentially separate groups of customers. The term typically refers to front-end servers, but some companies refer to database service pools for certain characteristics. Pools are typically X-axis-replicated (cloned) servers that are demarcated by function (Y axis) or customer (Z axis). 池是一些服务器，将相似功能分组或者潜在地分割客户群组。这个术语通常指的是前端服务器，但是一些公司指的数据库服务池。池通常是X轴复制（克隆）的服务器，并且能够通过功能（Y轴）或者客户（Z轴）加以区分。|
Shard| Shards are horizontal partitions of databases or search engines. Horizontal partitioning means the separation of data across database tables, database instances, or physical database servers. Shards typically occur along the Z axis of scale (for instance, splitting up customers among shards), but some companies refer to functional (Y axis) splits as shards as well. 分片是数据库或者搜索引擎的水平分区。水平分区意味着跨数据库表、数据库实例或者物理数据库的分割。分片通常出现在沿着Z轴扩展（例如，在分片之间拆分客户），但是一些公司也指的是将功能（Y轴）分拆为像片一样。|
Swim lane | Swim lane is a term used to identify a fault isolation domain. Synchronous calls are never allowed across swim lane boundaries. Put another way, a swim lane is defined around a set of synchronous calls. The failure of a component within one swim lane does not affect components in other swim lanes. As such, no component is shared across swim lanes. A swim lane is the smallest boundary across which no synchronous calls occur. 泳道是术语，用于识别一个故障隔离域。不许可跨域泳道边界的同步调用。换言之，围绕着一个同步调用集合定义一个泳道。一个泳道内的一个组件失效不会影响其他泳道的组件。因此，跨越泳道不会共享组件。一个泳道是跨越边界没有出现同步调用的最小边界。|
...
**How to use:** Develop shared libraries to allow automatic or on-demand enabling and disabling of features. See Table 9.5 for recommendations. 为了允许自动地或者按需地启用或者禁用特性，开发共享库。见表9.5中的建议。

**Why:** Turning off broken functionality or noncritical functionality in order to protect and preserve critical functionality is important for end users. 对于最终用户而言，关闭有问题的或者不关键的功能以保护和保留关键的功能是非常重要的。
...
在第7章“从你的错误中学习”中讨论回滚时，我们介绍了我们称之为开启/关闭框架的概念，并且在本章我们讨论将故障隔离作为一种设计方法时又再次谈及这一概念。这类框架最终有助于确保你的系统能够或者优雅地失败（在自诊断框架的情况下）或者在通过人为干预禁用特定功能情况下运行。有时候公司将类似的功能称为“特性切换器“或者”断路器“


Feature Wire-On/Wire-Off Approaches
|Approach方法 | Description描述 | Pro优点 |  Con缺点|
| :------------ | :------------ |:------------ |:------------ |
| Automatic markdown based on time-outs基于过期时间的自动下线 | When calls to third parties become slow, the application logic will mark this functionality as “off” for a period of time or until manual intervention re-enables it. 当对于第三方的调用变得缓慢时，应用逻辑将会标记这个功能为“断开”一段时间或者直到手工干预重新开启这个功能。| Fastest way to mark down a service that might bring several other services down when slow or unavailable.当一个服务变慢或者不可用时，可能会拖垮其他的几个服务，最快的标记此服务下线的方式。 | Sensitive to “false failures” or incorrect identification of a failing service. When coupled with auto markup, may cause a “pinging” effect of the service. Every service needs to make its own decision.对于“假的失效”或者没有正确识别一个正在失效的服务，非常敏感。当与自动上线结合使用，可能会对服务造成“乒乓”效应。每个服务都需要作出自己的决定。|
|Stand-in service 替代服务 |Replace a service with an auto responder with a dummy response that indicates service unavailable or a cached response of “likely good data.” 通过两种方式替换服务：其一是使用自动应答服务，其能够返回一个假的应答，以表明服务不可用；其二是使用一个被缓存的、“可能为好数据”的应答|Easy to implement, at least on the service side. May allow user determination of failure. 容易实现，至少是在服务侧是如此。可以许可由用户确定故障。| Each calling service needs to understand the “failure” response. May be slower to mark down and may require user intervention to mark up. 每次调用服务需要都理解“失效”的应答。可能使得标记下线变得更慢，并且可能需要用户干预来标记上线。|
|Manual markdown command 手工标记下线命令| Administrators manually send a signal/issue a command to stop displaying the functionality that is performing slowly. 管理员手工地发送一个信号/发出一个命令，停止运行当前执行缓慢的功能| Allows manual determination of failed service. 许可手工确定失败的服务。 | Slower than automatic markdown. Also, if services have TCP ports full due to slow or failed service, the command may not work as desired. 比自动标记下线要慢。此外，如果由于变慢的或者失败的服务，服务的TCP端口全部占满，则命令可能不会按照预期的那样工作。|
|Config file markdown 配置文件标记下线| Change configuration variable in a file to indicate the “wire off” of a feature. 为了指示关闭一个特性，更改文件中的配置变量。| Doesn’t rely on request/reply communication as in a command. 不依赖于像在命令中那样的请求/应答通信| Likely requires restart of a service to reload configurations in memory. 很可能需要重启服务来将配置加载到内存中。|
|File markdown 文件标注下线 | Presence of a file (or absence of a file) indicates if the functionality is available or not. 一个文件的存在（或者缺失）表明相对应的功能是否可用。 | Doesn’t rely on request/reply communication as in a synchronous command. Might not require service restart. 不依赖于像在同步命令中那样的请求/应答通信|May slow down processing as each request requires “polling” for files. 因为每个请求都需要轮训文件，从而可能拖慢下线处理。|
| Runtime variable 运行时变量| Read at startup as an argument to the program for daemonlike processes. 针对于守护进程，类似于针对程序的参数，在启动时读取| Similar to config file. 类似于配置文件|Similar to config file. 类似于配置文件|



In fact, many companies blend some of these options. They may read in variables from a database or a file at startup that direct the application code to display or not display certain sets of functionality. An example of this was implemented at PayPal when they first implemented internationalization. Some countries allowed only a limited set of payment functionality based on banking or money transfer regulations. Depending on the geographical location from which the user was accessing the site, he or she might see only a subset of the functionality provided on the main site.
实际上，许多公司对于这些方法混合使用。他们可能在启动时从数据库或者文件读取变量，指示应用代码显示或者不显示特定的功能集合。
...
当考虑针对于特性/功能的开启/关闭框架时，同等主要的并且需要处理的问题是决定何处和何时使用它们。很显然，实现这个框架意味着额外的工作以及为此会给业务带来额外的成本。让我们从某些特性永不失效（不太可能，也不正常）的情况开始。如果我们能够告知哪个特性永远不会失效，那么我们可能永远都不想针对此特性实现这个功能，因此其代表了没有回报的成本。从这个开始的情况，我们能够识别出哪里的投资有价值或者哪里能够给业务提供回报。在网站中任何具有高使用率（高吞吐）的和其失效可能影响其他重要特性的特性都是候选。另一个候选是任何在给定版本中进行了相当显著更改的特性。在上述两种情况中的思想是实现开启/关闭的成本要小于对于我们业务的风险（其中风险是失效的可能性和失效的影响的一个函数）。如果开发具有这个功能的特性需要花费额外的1000美元，而未处理此特性的失效可能在业务上花费我们10000美元，那么这个成本是否合理？
...
出于数个关键原因，我们建议实现开启/关闭框架。第一，当功能是新的或者正处于开发过程中，其更有可能出现bug。因此，能够标注下线或者关闭有问题的功能是非常值得做的。第二，如果对于你所提供的服务，这个功能并不关键，那么你可能想要能够标记下线或者关闭非关键性功能。当计算资源开始受到限制（可能是你有一个内存泄漏，从而将你的应用推入完全的垃圾回收中），能够关闭非关键性的功能来保留更多的关键性特性是非常好的。第三，对第三方调用经常不得不采用同步方式。当提供方的API开始应答缓慢时，是非常期望能够关闭这功能，以防止其拖慢你整个发应用或服务。要清楚的是，我们不认为所有的功能都应该能够被启用/禁用或开启/关闭，因为这样做的代价高昂，而且不明智，但是运行良好的团队应该能够识别风险和共享组件，以便实施适当的保障措施。
