...
实际上，许多公司混合使用数个方法。他们可能在启动时从数据库或者文件读取变量，指示应用代码显示或者不显示特定的功能集合。
...
当考虑针对于特性/功能的开启/关闭框架时，同等主要的、需要处理问题是决定何处和何时使用它们。很显然实现这个框架意味着额外的工作以及为此给业务带来额外的成本。让我们从某些特性永不失效（不太可能，也正确）的情况开始，我们可能永远都不想针对此特性实现这个功能，因此其代表了没有回报的成本。从这个开始的情况，我们能够识别出哪里的投资有价值或者哪里能够给业务提供回报。在网站中任何具有高使用率（高吞吐）的和其失效可能影响其他重要特性的特性都是候选。另一个候选是任何在给定版本中进行了相当显著更改的特性。在上述两种情况中的思想是实现开启/关闭的成本要小于对于我们业务的风险（其中风险是一个失效可能性和失效影响的函数）。如果开发具有这个功能的特性需要花费额外的1000美元，而未处理此特性的失效可能在业务上可能花费我们10000美元，那么这个成本是否合理？
...
出于数个关键原因，我们建议实现开启/关闭框架。第一，当功能是新的或者正处于开发过程中，其更有可能出现bug。因此，能够标注下线或者关闭有问题的功能是非常值得做的。第二，如果对于你所提供的服务，这个功能并不关键，那么你可能想要能够标记下线或者关闭非关键性功能。当计算资源开始受到限制（可能是你有一个内存泄漏，从而将你的应用推入完全的垃圾回收中），能够关闭非关键性功能来保留更多的关键性特性是非常好的。第三，对第三方调用经常不得不采用同步方式。当提供方的API开始应答缓慢时，是非常期望能够关闭这功能，以防止其拖慢你整个应用或服务。要清楚的是，我们不认为所有的功能都应该能够被启用/禁用或开启/关闭，因为这样做的代价高昂，而且不明智，但是运行良好的团队应该能够识别风险和共享组件，以便实施适当的保障措施。
