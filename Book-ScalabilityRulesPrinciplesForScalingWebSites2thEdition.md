...
| :------------ | :------------ |  
|Pod |Pods are self-contained sets of functionality containing app servers, persistent　storage (such as a database or other persistent and shared file system), or　both. Pods are most often splits along the Z axis, as in a split of customers　into separate pods. Pod is sometimes used interchangeably with the term swim　lane. It has also been used interchangeably with the term pool when referring　to Web or application services.Pods是自包含的功能集合，包括应用服务器或者持久存储（例如数据库或者其他持久的和共享的文件系统），或者同时包含两者。Pods通常是沿着Z轴拆分，就像将客户拆分到相互分离的pods中。Pod优势也与术语泳道相互交换使用。当指的是Web或者应用时，也可以与术语池相互交换使用。|
|Cluster | Clusters are sometimes used for Web and application servers in the same fashion as a “pool,” identified next. In these cases a cluster refers to an X axis scale of similar functionality or purpose configured such that all nodes or participants are “active.” Often a cluster will share some sort of distributed state above and beyond that of a pool, but this state can cause scalability bottlenecks under high transaction volumes. Clusters might also refer to active/passive configuration where one or more devices sit “passive” and become “active” on the failure of a peer device.针对于Web或者应用服务器，有时会以像池一样的方式使用集群。 在这些例子中，一个集群指的是通过配置所有的节点或者参与者处于活跃状态来实现针对相似功能或者目的X轴可扩展。通常一个集群会共享一些类型的状态，这些状态在池之上或者超过池，并且这些状态在大量事务时能够导致可扩展性瓶颈。集群可能也指的是主动/被动配置，其一个或者多个设备节点处于被动状态，并且当对等设备故障时变为主动状态。|  
|Pool | Pools are servers that group similar functionality or potentially separate groups of customers. The term typically refers to front-end servers, but some companies refer to database service pools for certain characteristics. Pools are typically X-axis-replicated (cloned) servers that are demarcated by function (Y axis) or customer (Z axis). 池是将相似功能分组或者潜在地分割客户组的服务器。这个术语通常指的是前端服务器，但是一些公司指的数据库服务池。池通常是X轴复制（克隆）的服务器，并且能够通过功能（Y轴）或者客户（Z轴）加以区分。|
