...
| :------------ | :------------ |:------------ |:------------ |
| Automatic markdown based on time-outs基于过期时间的自动降级 | When calls to third parties become slow, the application logic will mark this functionality as “off” for a period of time or until manual intervention re-enables it. 当对于第三方的调用变得缓慢时，应用逻辑将会标记这个功能为“断开”或者直到手工干预重新开启这个功能。| Fastest way to mark down a service that might bring several other services down when slow or unavailable.当一个服务变慢或者不可用时，可能会拖垮其他的几个服务，最快的标记此服务下线的方式。 | Sensitive to “false failures” or incorrect identification of a failing service. When coupled with auto markup, may cause a “pinging” effect of the service. Every service needs to make its own decision.对于“假的失效”或者不正确地识别一个正在失效的服务，非常敏感。当与自动上线结合使用，可能会对服务造成“乒乓”效应。每个服务都需要作出自己的决定。|
|Stand-in service 替代服务 |Replace a service with an auto responder with a dummy response that indicates service unavailable or a cached response of “likely good data.” 通过两种方式替换服务：其一是使用自动应答服务，其能够返回一个假的应答，以表明服务不可用；其二是使用一个被缓存的、“可能为好数据”的应答|Easy to implement, at least on the service side. May allow user determination of failure. 容易实现，至少是在服务侧是如此。可用许可由用户确定故障。| Each calling service needs to understand the “failure” response. May be slower to mark down and may require user intervention to mark up. 每次调用服务需要理解“失效”的应答。可能使得标记下线变得更慢，并且可能需要用户干预来标记上线。|
|Manual markdown command 手工标记下线命令| Administrators manually send a signal/issue a command to stop displaying the functionality that is performing slowly. 管理员手工地发生一个信号/发出一个命令，停止运行当前执行缓慢的功能| Allows manual determination of failed service. 许可手工确定失败的服务。 | Slower than automatic markdown. Also, if services have TCP ports full due to slow or failed service, the command may not work as desired. 比自动标记下线要慢。此外，如果由于变慢的或者失败的服务，服务的TCP端口全部占满，则命令可能不会按照预期的工作。|
|Config file markdown 配置文件标记下线| Change configuration variable in a file to indicate the “wire off” of a feature. 更改文件中的配置便利，用于指示关闭一个特性。| Doesn’t rely on request/reply communication as in a command. 不依赖于类似于在命令中请求/应答通信| Likely requires restart of a service to reload configurations in memory. 很可能需要重启服务来将配置加载到内存中。|
|File markdown 文件标注下线 | Presence of a file (or absence of a file) indicates if the functionality is available or not. 一个文件的存在（或者缺失）表明相对应的功能是否可用。 | Doesn’t rely on request/reply communication as in a synchronous command. Might not require service restart. 不依赖于类似在同步命令中的请求/应答通信|May slow down processing as each request requires “polling” for files. 因为每个请求都需要轮训文件，从而可能拖慢下线处理。|
| Runtime variable 运行时变量| Read at startup as an argument to the program for daemonlike processes. 针对于守护进程，类似于，在启动时读取| Similar to config file. 类似于配置文件|Similar to config file. 类似于配置文件|
